...
bool valid()
{
   ...
   for (int l1 = 1; l1 <= n; l1++) //parcurgen matricea
   {
      for (int c1 = 1; c1 <= n; c1++)
      {
         if (a[l1][c1] == 1) // cand am gasit 1
         {
            ...
            //verificam daca nu exista 1 pe diagonala
   ...
   return true; //daca nu gasim, spunem ca este valid
}
...
void tip() //funtie de afisare
{
   ...
   {
      if (l2 != 1) //afiseaza endl inainte de fiecare linie inafara de prima, in unenele probleme trebuie facut asa
      {
         fout << endl;
      }
...
{
   fin >> n;
   for (int l2 = 1; l2 <= n; l2++) //initializam matricea(obligatoriu daca faceam cu matrice de tip char dar aici nu e nevoie)
   {
      for (int c2 = 1; c2 <= n; c2++)
      {
         a[l2][c2] = 0;
